...
class BraceParse(HasTraits):
    ...
    def __init__(
        ...
        enforce_valid_types(comment_delimiters, (list,), "comment_delimiters parameter must be a list.")
    ...
    def parse_braces_to_nested_list(self, config_txt: str):
        ...
        try:
            ...
            valid_chars = Combine(OneOrMore(dblQuotedString() | Word(printables, exclude_chars="{}") | White(" ")))
            if self.debug:
...
class ConfigList(UserList):
    ...
    def __init__(
        ...
        self.data: list[BaseCfgLine] = []

        ####################################################################
        # Store initlist in self.data if initlist is a valid Sequence
        ####################################################################
        if len(initlist) > 0 and getattr(initlist, "__iter__", False):
            self.data = self.bootstrap(text_list=initlist, debug=debug)
    ...
    # This method is on ConfigList()
    @logger.catch(reraise=True)
    def __enter__(self) -> GeneratorType:
    ...
    def remove(self, item: BaseCfgLine) -> None:
        ...
        if isinstance(item, BaseCfgLine):
            ...
            error = f"item must be an instance of BaseCfgLine(), but we got {type(item)}"
            logger.critical(error)
            raise InvalidParameters(error)
        ...
        for parent in banner_objs:
            ...
            while bannerdelimit is not None:
                ...
                try:
                    ...
                    if obj.text is None:
                        if self.debug > 0:
                            logger.warning(f"found empty text while parsing '{obj}' in the banner")
    ...
    def _maintain_bootstrap_parent_cache(
        ...
        max_indent: int,
        is_config_line: bool,
    ) -> tuple[dict[int, BaseCfgLine], BaseCfgLine]:
    ...
    def _build_bootstrap_parent_child(
        ...
        obj: BaseCfgLine,
        debug: int,
    ) -> tuple[list[BaseCfgLine], dict, BaseCfgLine]:
        ...
        if (indent > 0) and (parent is not None):
            ...
            while candidate_parent_idx >= 0:
                ...
                if (candidate_parent.indent < indent) and candidate_parent.is_config_line:
                    # We found the parent
    ...
    # This method is on ConfigList()
    @logger.catch(reraise=True)
    def bootstrap(self, text_list: list[str] | tuple[str, ...] | None = None, debug: int = 0) -> list[BaseCfgLine]:
        """
        ...
        if not isinstance(text_list, Sequence):
            error = f"Invalid ConfigList() param type text_list: {type(text_list)}"
            logger.error(error)
            raise ValueError(error)
        ...
        for idx, txt in enumerate(text_list):
            ...
            if not isinstance(txt, str):
                error = f"ConfigList().bootstrap() can only digest strings, not {type(txt)}"
                logger.error(error)
                raise ValueError(error)
            ...
            parents_cache, parent = self._maintain_bootstrap_parent_cache(parents_cache, indent, max_indent, is_config_line)
        ...
        if self.ignore_blank_lines is True:
            retval = [obj for obj in self.data if obj.text.strip() != "" or obj.blank_line_keep is True]
            self.data = retval
    ...
    # This method is on ConfigList()
    @logger.catch(reraise=True)
    def iter_with_comments(self, begin_index: int = 0) -> GeneratorType:
        """
    ...
    # This method is on ConfigList()
    @logger.catch(reraise=True)
    def iter_no_comments(self, begin_index: int = 0) -> GeneratorType:
        """
...
class CiscoConfParse:
    ...
    # This method is on CiscoConfParse()
    @logger.catch(reraise=True)
    def auto_indent_configuration(self, indent_width: int | None = None) -> bool:
        """
        Use the auto_indent_width to automatically indent the configuration.

        This action automatically commits the configuration.

        indent_width is an optional integer parameter.  Values
        for indent_width should be greater than 1.
        """
    ...
    # This method is on CiscoConfParse()
    @logger.catch(reraise=True)
    def handle_ccp_brace_syntax(self, tmp_lines: list | None = None, syntax: str | None = None) -> list[str]:
    ...
    # This method is on CiscoConfParse()
    @logger.catch(reraise=True)
    def read_config_file(self, filepath: str | None = None, linesplit_rgx: str = r"\r*\n") -> list[str]:
    ...
    def check_ccp_input_good(
        ...
        if isinstance(config, Sequence):
            ...
            if self.debug > 0:
                _logger.debug(f"parsing config stored in the config variable: `{config}`")
            return True
...
debug={debug},
    ...
    def find_objects(
        ...
        if self.debug > 0:
            logger.info(f"find_objects('{linespec}', exactmatch={exactmatch}) was called")
    ...
    def find_parent_objects_wo_child(
        ...
        return [obj for obj in self.find_objects(parentspec, ignore_ws=ignore_ws, escape_chars=False, reverse=reverse) if not obj.find_child_objects(childspec, recurse=recurse)]
    ...
    def _find_line_OBJ(self, linespec, exactmatch=False):
        ...
        if self.debug >= 2:
            logger.debug(f"Looking for match of linespec='{linespec}', exactmatch={exactmatch}")
...
class CiscoPassword(HasTraits):
    ...
    def __init__(self, ep=""):
        super().__init__()
        if ep != "":
            logger.warning("In the future, calling CiscoPassword() with a string will be deprecated.")
        self.ep = ep
...
def config_line_factory(
    all_lines: list[str] | list[BaseCfgLine] | None = None,
    line: str | BaseCfgLine | None = None,
    index: int | None = None,
    comment_delimiters: list[str] | None = None,
    syntax: str = "ios",
    debug: int = 0,
