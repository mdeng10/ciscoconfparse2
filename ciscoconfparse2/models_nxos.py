...
##
##-------------  NXOS Configuration line object
##
...
class NXOSCfgLine(BaseFactoryLine):
    ...
    def from_list(cls, *list_of_args) -> BaseCfgLine:
        """Helper-method to allow strictly positional *arg calls .i.e. NXOSCfgLine([], 'hostname Foo')"""
...
class BaseNXOSIntfLine(NXOSCfgLine, BaseFactoryInterfaceLine):
    ...
    def has_no_ip_proxyarp(self) -> bool:
        ...
        ## By default, Cisco NXOS answers proxy-arp
        ## By default, Nexus disables proxy-arp
        ## By default, NXOS-XR disables proxy-arp
    ...
    def manual_arp_timeout(self) -> int:
        ...
        ## By default, Cisco NXOS defaults to 4 hour arp timers
...
class NXOSIntfLine(BaseNXOSIntfLine):
    ...
    @logger.catch(reraise=True)
    def __init__(self, *args, **kwargs):
        r"""Accept an NXOS line number and initialize family relationship
        attributes
...
class NXOSIntfGlobal(NXOSCfgLine):
    ...
    @logger.catch(reraise=True)
    def __init__(self, *args, **kwargs):
        super(NXOSIntfGlobal).__init__(*args, **kwargs)
...
class NXOSAccessLine(NXOSCfgLine):
    ...
    def name(self):
        retval = self.re_match_typed(r"^line\s+(\S+)", result_type=str, default="")
        # special case for NXOS async lines: i.e. "line 33 48"
